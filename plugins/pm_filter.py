...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        try:
            hehe = await message.reply_photo(photo=imdb.get('poster'), caption=str(cap[:1024]), reply_markup=InlineKeyboardMarkup(btn))
            try:
                if settings['auto_delete']:
            ...
            hmm = await message.reply_photo(photo=poster, caption=str(cap[:1024]), reply_markup=InlineKeyboardMarkup(btn))
            try:
                if settings['auto_delete']:
                    await asyncio.sleep(600)
                    await hmm.delete()
